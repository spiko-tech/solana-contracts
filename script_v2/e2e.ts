/**
 * End-to-end multi-actor test on devnet (v2).
 *
 * Uses Codama-generated instruction builders instead of hand-written ones.
 * The only hand-written instruction is buildTransferChecked (Path A),
 * which is a raw Token-2022 TransferChecked, not a Spiko program ix.
 *
 * 6 accounts, 10 steps (7a/7b):
 ...
 *   User1       — ROLE_WHITELISTED (bit 4)
 *   User2       — ROLE_WHITELISTED (bit 4)
 *
 * Flow:
 ...
 *   7b. User1 transfers 5 shares to User2 via Token-2022 TransferChecked (Path A)
...
import { getGrantRoleInstruction } from "../clients/ts/permission-manager/instructions/grantRole.js";
import { getInitiateMintInstruction } from "../clients/ts/minter/instructions/initiateMint.js";
import { getTransferTokenInstruction } from "../clients/ts/spiko-token/instructions/transferToken.js";
import { getRedeemTokenInstruction } from "../clients/ts/spiko-token/instructions/redeemToken.js";
import { getExecuteRedemptionInstruction } from "../clients/ts/redemption/instructions/executeRedemption.js";
...
async function main() {
  console.log("=== Spiko E2E Multi-Actor Test (v2 — Codama) ===\n");
  ...
  await step("Admin grants ROLE_MINT_INITIATOR to Minter", ["RoleGranted"], async () => {
    const ix = getGrantRoleInstruction({
      caller: admin,
      permissionConfig: permConfigAddr,
      userPermissions: minterPermsAddr,
  ...
  await step("Admin grants ROLE_REDEMPTION_EXECUTOR to Executor", ["RoleGranted"], async () => {
    const ix = getGrantRoleInstruction({
      caller: admin,
      permissionConfig: permConfigAddr,
      userPermissions: executorPermsAddr,
  ...
  await step("Admin grants ROLE_WHITELISTER to Whitelister", ["RoleGranted"], async () => {
    const ix = getGrantRoleInstruction({
      caller: admin,
      permissionConfig: permConfigAddr,
      userPermissions: whitelisterPermsAddr,
  ...
  await step("Whitelister grants ROLE_WHITELISTED to User1", ["RoleGranted"], async () => {
    const ix = getGrantRoleInstruction({
      caller: whitelister,
      permissionConfig: permConfigAddr,
      userPermissions: user1PermsAddr,
  ...
  await step("Whitelister grants ROLE_WHITELISTED to User2", ["RoleGranted"], async () => {
    const ix = getGrantRoleInstruction({
      caller: whitelister,
      permissionConfig: permConfigAddr,
      userPermissions: user2PermsAddr,
  ...
  await step(
    ...
    async () => {
      ...
      if (!(await accountExists(rpc, user1Ata))) {
        instructions.push(
          createAssociatedTokenAccountIdempotent(admin, user1Ata, user1.address, mintAddr)
        );
      }
      ...
      instructions.push(
        getInitiateMintInstruction({
          ...
          callerPermissions: minterPermsAddr,
          ...
          minterUserPermissions: minterConfigPermsAddr,
          recipientPermissions: user1PermsAddr,
          user: user1.address,
  ...
  await step(
    ...
    async () => {
      ...
      if (!(await accountExists(rpc, user2Ata))) {
        instructions.push(
          createAssociatedTokenAccountIdempotent(admin, user2Ata, user2.address, mintAddr)
        );
      }
      ...
      instructions.push(
        getTransferTokenInstruction({
          ...
          mint: mintAddr,
          senderPermissions: user1PermsAddr,
          recipientPermissions: user2PermsAddr,
  ...
  await step(
    ...
    async () => {
      ...
      if (!(await accountExists(rpc, vaultAta))) {
        instructions.push(
          createAssociatedTokenAccountIdempotent(admin, vaultAta, vaultAuthAddr, mintAddr)
        );
      }
      ...
      instructions.push(
        getRedeemTokenInstruction({
          ...
          mint: mintAddr,
          userPermissions: user2PermsAddr,
          vaultAuthorityPermissions: vaultAuthPermsAddr,
  ...
  await step(
    ...
    async () => {
      ...
      const ix = getExecuteRedemptionInstruction({
        ...
        redemptionConfig: redemptionConfigAddr,
        redemptionOperation: redemptionOpAddr,
        operatorPermissions: executorPermsAddr,
        spikoTokenProgram: SPIKO_TOKEN_PROGRAM_ADDRESS as Address,
        tokenConfig: tokenConfigAddr,
        tokenMint: mintAddr,
        vault: vaultAta,
