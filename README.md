...
Five programs enforce role-based access control, whitelist-gated transfers, controlled minting with daily limits, and admin-forced burns via PermanentDelegate. See [ARCHITECTURE.md](./ARCHITECTURE.md) for the full system design.
...
| **SpikoTransferHook**    | Transfer hook program invoked by Token-2022 on every transfer. Enforces whitelist checks for both sender and recipient.                              |
...
| SpikoTransferHook    | `GMpqabVyd98sefjUB3fLhy6HA77xTnGpWy9e7vJs2vtJ` |
...
cargo build-sbf -- -p spiko-transfer-hook
...
## IDL & Client Generation (Codama)

IDLs are hand-written Anchor v0.1 JSON files in `idl/`:

| File                         | Program            |
| ---------------------------- | ------------------ |
| `permission_manager.json`    | PermissionManager  |
| `spiko_token.json`           | SpikoToken         |
| `spiko_transfer_hook.json`   | SpikoTransferHook  |
| `minter.json`                | Minter             |
| `redemption.json`            | Redemption         |

TypeScript clients are generated from these IDLs using [Codama](https://github.com/codama-idl/codama). Install dependencies first:

```bash
pnpm install       # root package.json — installs codama, @codama/nodes-from-anchor, @codama/renderers-js
```

Generate all five TypeScript clients:

```bash
node codama.mjs
```

Output lands in `clients/ts/<program-name>/` (one directory per program). These files are auto-generated — do not edit them directly. Re-run `node codama.mjs` after any IDL change.
...
Integration tests (end-to-end flows across all 5 programs):
...
E2E tests (v2, using Codama-generated clients):

```bash
cd script_v2 && pnpm install && pnpx tsx e2e.ts
```
...
```bash
pnpx tsx whitelist.ts <USER_ADDRESS>
```

Example:

```bash
pnpx tsx whitelist.ts 7xKXtg2CW87d97TXJSDpbD5jBkheTqA83TZRuJosgAsU
```
...
```bash
pnpx tsx mint.ts <RECIPIENT_ADDRESS> <AMOUNT> <TOKEN>
```
...
```bash
pnpx tsx mint.ts 7xKXtg2CW87d97TXJSDpbD5jBkheTqA83TZRuJosgAsU 100 eutbl
```
...
```bash
pnpx tsx burn.ts <HOLDER_ADDRESS> <AMOUNT> <TOKEN>
```

Example — burn 50 USTBL from a holder:

```bash
pnpx tsx burn.ts 7xKXtg2CW87d97TXJSDpbD5jBkheTqA83TZRuJosgAsU 50 ustbl
```
...
```bash
pnpx tsx redeem.ts <eutbl|ustbl> <AMOUNT_IN_SHARES> [SALT]
```

Example — redeem 10 EUTBL:

```bash
pnpx tsx redeem.ts eutbl 10
```
...
```bash
pnpx tsx execute-redemption.ts <eutbl|ustbl> <USER_ADDRESS> <AMOUNT_IN_SHARES> <SALT>
```

Example:

```bash
pnpx tsx execute-redemption.ts eutbl 3QpaUQVDFTKKK56pDRxmVcX1rMjEjXRcLECmeAaGs9Sx 10 12345
```
...
```bash
pnpx tsx cancel-redemption.ts <eutbl|ustbl> <USER_ADDRESS> <AMOUNT_IN_SHARES> <SALT>
```

Example:

```bash
pnpx tsx cancel-redemption.ts eutbl 3QpaUQVDFTKKK56pDRxmVcX1rMjEjXRcLECmeAaGs9Sx 10 12345
```

## Event System

All five programs emit **Anchor-compatible structured events** via the `sol_log_data` syscall. Events appear in transaction logs as `Program data: <base64>` entries and can be decoded by any Anchor-compatible tooling.
...
| `RoleGranted`                  | `event:RoleGranted`                  | `dcb759e48f3ff63a` | `caller: pubkey(32), target: pubkey(32), roleId: u8(1)`   |
| `RoleRemoved`                  | `event:RoleRemoved`                  | `8523d6eacb9dcb35` | `caller: pubkey(32), target: pubkey(32), roleId: u8(1)`   |
| `OwnershipTransferStarted`     | `event:OwnershipTransferStarted`     | `b7fdeff68cb38569` | `admin: pubkey(32), newAdmin: pubkey(32)`                  |
| `OwnershipTransferred`         | `event:OwnershipTransferred`         | `ac3dcdb7fa322662` | `newAdmin: pubkey(32)`                                     |
...
| `Mint`                  | `event:Mint`                  | `3f0bd58694c218cb` | `caller: pubkey(32), mint: pubkey(32), recipientAta: pubkey(32), amount: u64(8)` |
| `Burn`                  | `event:Burn`                  | `b80d41ceceaa3355` | `caller: pubkey(32), mint: pubkey(32), sourceAta: pubkey(32), amount: u64(8)`  |
...
| Event               | Hash Input                | Discriminator      | Fields                                                                                   |
| ------------------- | ------------------------- | ------------------ | ---------------------------------------------------------------------------------------- |
| `MinterInitialized` | `event:MinterInitialized` | `b18962b316ce37c0` | `admin: pubkey(32), maxDelay: i64(8)`                                                    |
...
| `MaxDelayUpdated`   | `event:MaxDelayUpdated`   | `8151911a62d2a00c` | `caller: pubkey(32), maxDelay: i64(8)`                                                   |
...
| `RedemptionInitiated`   | `event:RedemptionInitiated`   | `55feeb0edd8860de` | `user: pubkey(32), mint: pubkey(32), amount: u64(8), salt: u64(8), deadline: i64(8)`     |
...
#### SpikoTransferHook (1 event)

| Event      | Hash Input         | Discriminator      | Fields                                                                                         |
| ---------- | ------------------ | ------------------ | ---------------------------------------------------------------------------------------------- |
| `Transfer` | `event:Transfer`   | `19121707ac74821c` | `sender: pubkey(32), mint: pubkey(32), source: pubkey(32), destination: pubkey(32), amount: u64(8)` |
